...
export async function extractOpenGraphData(url: string, clientUserAgent?: string): Promise<MetadataResult> {
  ...
  try {
    ...
    if (result.title) {
      // Primero limpiamos entidades HTML y espacios
      ...
      // Eliminar nombres de dominio específicos y frecuentes
      result.title = cleanStoreDomainsFromTitle(result.title, url);
    }
 ...
 */
function cleanStoreDomains(title: string, sourceUrl: string): string {
