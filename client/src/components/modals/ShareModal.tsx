...
const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, shareableLink }) => {
  ...
  const copyToClipboard = () => {
    if (linkRef.current) {
      ...
      try {
        ...
        // Mostrar también el toast usando el hook
        toast({
          title: "Enlace copiado al portapapeles",
          description: "Ya puedes compartirlo con quien quieras"
        });
        
        // Quitar el toast después de un tiempo
        setTimeout(() => {
          setShowCopiedToast(false);
        }, 3000);
      } catch (e) {
        // Intentar con la API moderna como fallback
  ...
  const shareByEmail = () => {
    ...
    // Usar window.location.href directamente para asegurar que se abre el cliente de correo predeterminado
  ...
  return (
    <>
      {/* Toast personalizado directo */}
      {showCopiedToast && (
        <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 z-[9999] px-5 py-3 bg-[#152218] text-white border border-green-800/30 rounded-lg shadow-lg flex items-center justify-between transition-opacity duration-300">
          <div className="grid gap-1">
            <div className="font-medium text-sm">Enlace copiado al portapapeles</div>
            <div className="text-xs opacity-90">Ya puedes compartirlo con quien quieras</div>
          </div>
        </div>
      )}
