        ...
        <div className="w-32 h-32 shrink-0 relative">
          {item.imageUrl ? (
            <img 
              src={item.imageUrl} 
              alt={item.name} 
              className={`w-full h-full object-cover ${item.isReserved && !isReservedByMe ? 'filter grayscale' : ''}`} 
            />
          ) : (
            <div className={`w-full h-full flex items-center justify-center bg-gray-100 ${item.isReserved && !isReservedByMe ? 'filter grayscale' : ''}`}>
